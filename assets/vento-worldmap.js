...
(function () {
  ...
  const HOTSPOTS = [
    ...
    { id:'suedkorea',  lon:127.5,  lat:37.0, labelAnchor:'start', labelDx:9,   labelDy:-8, mobileAnchor:'end', mobileDx:-9 },
  ...
  function build(root) {
    ...
    HOTSPOTS.forEach(s => {
      ...
      const anchor = (mobile && s.mobileAnchor) ? s.mobileAnchor : s.labelAnchor;
      const dx = (mobile && s.mobileDx !== undefined) ? s.mobileDx : s.labelDx;
      const label = el('text', {
        x: sx, y: sy - 13, class: 'vw-spot-label',
        'text-anchor': anchor || (sx > VB_W - 150 ? 'end' : (sx < 120 ? 'start' : 'middle'))
      });
      if (dx) label.setAttribute('dx', dx);
      ...
      if (s.hub) {
        const brand = el('text', {
          x: sx, y: sy - 13,
          dy: window.matchMedia('(max-width: 720px)').matches ? -92 : -46,
          'text-anchor': 'middle',
